...
economic_indices = [
    ...
    ['ИНДЕКС ПОТРЕБИТЕЛЬСКИХ ЦЕН И ИНДЕКС ЦЕН ПРОИЗВОДИТЕЛЕЙ', 'cpi_ppi', 'МАКРОЭКОНОМИКА'],
    ...
    ['ОСНОВНЫЕ НЕСЫРЬЕВЫЕ ЭКСПОРТНЫЕ ТОВАРЫ, $МЛН', 'major_non_raw_export_goods_in_million_usd', 'МАКРОЭКОНОМИКА'],
    ...
    ['КОНСЕНСУС ПРОГНОЗ РОСТА ВВП КАЗАХСТАНА И ДРУГИХ СТРАН (%) И ЦЕН НА НЕФТЬ ($/БАРРЕЛЬ)', 'consensus_gdp_growth_and_oil_prices_forecast', 'МАКРОЭКОНОМИКА'],
